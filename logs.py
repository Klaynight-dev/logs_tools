...
def log_action(action, success=True, error_message=None, empr=False):
  ...
  """
    ...
    with open(log_filename, 'a') as log_file:
        log_file.write(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - Action: {action} - Success: {'Yes' if success else 'No'} -","Error: {error_message}\n" if !success else "\n")
        if not success and error_message:
            log_file.write(f"\n")
    if empr==True:
        print(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - Action: {action} - Success: {'Yes' if success else 'No'} -","Error: {error_message}" if !success else "")
